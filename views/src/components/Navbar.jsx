  ...
  return (
    <>
      <nav className="w-full px-8 md:px-28 lg:px-38 py-4 flex justify-between items-center">
        <div className="text-[1.5rem] font-bold">Decora</div>
        ...
        <div className="flex items-center space-x-4">
          <div className="lg:w-full mx-auto hidden md:flex items-center lg:justify-between space-x-4">
            ...
            <div className="relative cursor-pointer" onClick={() => handleNavigate("/cart")}>
              ...
              <span className="absolute -top-1 -right-2 bg-black text-white text-[10px] w-4 h-4 flex items-center justify-center rounded-full">2</span>
            </div>
          </div>
      ...
      {menuOpen && (
        <div className="fixed inset-0 z-50 flex">
          <div className="w-[10%] h-screen bg-black/30 backdrop-blur-sm" onClick={() => setMenuOpen(false)}></div>
          ...
          <div className="w-[90%] h-screen bg-white p-6 flex flex-col justify-between overflow-y-auto">
            <div>
              ...
              <div className="flex items-center border px-3 py-3 rounded-md mb-6">
                ...
                <input type="text" placeholder="Search" className="w-full text-xs outline-none" />
              </div>

              <div className="flex flex-col space-y-4 text-sm font-medium text-gray-800">
                <Link className="border-b pb-4 border-gray-300" to="/" onClick={() => setMenuOpen(false)}>Home</Link>
                <Link className="border-b pb-4 border-gray-300" to="/shop" onClick={() => setMenuOpen(false)}>Shop</Link>
                <Link className="border-b pb-4 border-gray-300" to="/products" onClick={() => setMenuOpen(false)}>Products</Link>
                <Link className="border-b pb-4 border-gray-300" to="/contact" onClick={() => setMenuOpen(false)}>Contact Us</Link>
              </div>
            </div>

            <div>
              <div className="mt-6 space-y-4 text-sm">
                <div className="flex justify-between items-center border-b pb-4 border-gray-300 cursor-pointer" onClick={() => handleNavigate("/cart")}>
                ...
                <button className="w-full bg-black text-white py-2 rounded-md font-medium">Sign In</button>
              </div>
